    ...
    formatResponse(response) {
        if (response) {
            if (typeof response === 'string')
                response = JSONTryParse(response);
...
class Network extends Component {
    ...
    ListHeaderComponent = () => {
        ...
        <View style={styles.filterValueBar}>
          <TextInput ref={ref => {
                this.textInput = ref;
            }} style={styles.filterValueBarInput} placeholderTextColor="#000000a1" placeholder="After entering the content, please submit to filter..." onSubmitEditing={({ nativeEvent }) => {
                if (nativeEvent) {
                    this.regInstance = new RegExp(nativeEvent.text, 'ig');
                    this.setState({ filterValue: nativeEvent.text });
                }
            }}/>
          <TouchableOpacity style={styles.filterValueBarBtn} onPress={this.clearFilterValue}>
            <Text>X</Text>
          </TouchableOpacity>
        </View>
      </View>);
    ...
    retryFetch(item) {
        ...
        if (item.reqHeaders)
            options.headers = item.reqHeaders;
        if (item.method === 'POST' && item.postData)
    ...
    renderItem = ({ item }) => {
        ...
        if (this.state.filterValue && this.regInstance && !this.regInstance.test(_item.url))
            return null;
        return (<View style={styles.nwItem}>
        ...
        {this.state.showingId === _item.id && (<View style={styles.nwItemDetail}>
            <View>
            ...
            {_item.reqHeaders && (<View>
                <Text style={[styles.nwItemDetailHeader, styles.bold]}>Request Header</Text>
                {Object.keys(_item.reqHeaders).map(key => (<View style={styles.nwDetailItem} key={key}>
                    <Text>{key}:</Text>
                    <Text>{_item.reqHeaders[key]}</Text>
                  </View>))}
              </View>)}
            {_item.resHeaders && (<View>
                <Text style={[styles.nwItemDetailHeader, styles.bold]}>Response Header</Text>
                {Object.keys(_item.resHeaders).map(key => (<View style={styles.nwDetailItem} key={key}>
                    <Text>{key}:</Text>
            ...
            {_item.getData && (<View>
                ...
                {Object.keys(_item.getData).map(key => (<View style={styles.nwDetailItem} key={key}>
                    <Text>{key}:</Text>
            ...
            {_item.postData && (<View>
                ...
                <JsonTree theme={theme} data={JSONTryParse(_item.postData)} hideRoot/>
              </View>)}
            <View>
              <Text style={[styles.nwItemDetailHeader, styles.bold]}>Response</Text>
              <View style={styles.nwDetailItem}>
                <JsonTree theme={theme} data={JSONTryParse(_item.response)} hideRoot/>
              </View>
            </View>
          </View>)}
      </View>);
    ...
    render() {
        return (<FlatList ref={ref => {
                this.flatList = ref;
            }} showsVerticalScrollIndicator ListHeaderComponent={this.ListHeaderComponent} extraData={this.state} data={this.state.requestIds} stickyHeaderIndices={[0]} renderItem={this.renderItem} ListEmptyComponent={() => <Text> Loading...</Text>} keyExtractor={item => item}/>);
    }
}
